...
<div class="card">
        ...
        <!-- /.card-header -->
        <div class="card-body">   
        <form method="POST"  @can('isadmin')action="{{url('grades2') }}"@endcan  @can('isteacher')action="{{url('sgrades2') }}"@endcan >
            @csrf	
    ...
    <table class="table table-striped">
        ...
        @foreach($sloads as $sload)
             <tr>  
                    ...
                    <td>{{$sload->fname}} {{$sload->lname}}</td>
                    <td> {{$sload->subject}}  <input type="hidden" name="ssid[]" value={{"$sload->id"}}></td>                    
                    <td><input class="form-control" type="text" name="grade[]" value=""></td>               
        @endforeach
    @else
        <p>No posts found</p>
    @endif   
